...
export async function POST(req: Request) {
  try {
    ...
    const FROM = env.EMAIL_FROM || 'Mark App <onboarding@resend.dev>';
    ...
    let finalTo: string;
    if (to && env.EMAIL_ALLOWLIST) {
      const allowlist = env.EMAIL_ALLOWLIST.split(',').map(s => s.trim().toLowerCase());
      if (allowlist.includes(to.toLowerCase())) {
        finalTo = to;
      } else {
        return NextResponse.json({ error: 'Recipient not in allowlist' }, { status: 403 });
      }
    } else if (env.EMAIL_ALLOWLIST) {
      // Use first email in allowlist as default
      finalTo = env.EMAIL_ALLOWLIST.split(',')[0].trim();
    } else {

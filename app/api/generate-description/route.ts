...
function validateImage(base64Data: string): { valid: boolean; error?: string; sizeKB?: number } {
  try {
    // Remove data URL prefix if present
    const base64Only = base64Data.replace(/^data:image\/[a-z]+;base64,/, '');
...
export async function POST(request: NextRequest) {
  try {
    ...
    try {
      ...
      if (text.length > 20 * 1024 * 1024) { // 20MB limit
        return NextResponse.json(
    ...
    for (const imageData of selectedImages) {
      try {
        ...
        if (totalSizeKB > 10 * 1024) { // 10MB total limit
          return NextResponse.json(
    ...
    const textContent = {
      type: 'text' as const,
      text: `Produce a JSON object with keys:
...
Output strictly as JSON. Do not include any prose outside the JSON.`
    ...
    const completion = await openai.chat.completions.create({
      ...
      messages: [
        {
          role: 'system',
          content: 'You are an expert auction cataloger for antiques & vintage items. Be conservative: if unsure, say "unknown". Never invent maker names or dates. Keep language clear, neutral, and buyer-friendly.'
        },
        {
          role: 'user',
          content: userContent
    ...
    if (!parsedResponse.title || !parsedResponse.description_bullets || !parsedResponse.keywords || !parsedResponse.caution) {
    ...
    if (error instanceof OpenAI.APIError) {
      if (error.status === 429) {
        return NextResponse.json(
          { success: false, error: 'OpenAI rate limit reached. Please try again later.' },
          { status: 429 }
        );
      }
      if (error.status === 400) {
